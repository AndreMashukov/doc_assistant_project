...
def create_calculator_tool(logger: ToolLogger):
    """
    Creates a calculator tool that safely evaluates basic math expressions.
    """
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A mathematical expression string (e.g., "1500 + 2500", "(100 * 0.08)")

        Returns:
            A formatted string with the calculation result, or an error message
        """
        try:
            # Only allow digits, whitespace, and basic math operators
            if not re.match(r'^[\d\s+\-*/().%]+$', expression):
                error_msg = (
                    "Invalid expression: only numbers and basic math operations "
